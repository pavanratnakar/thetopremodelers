...
class CategorySection extends General{
    public function getDetails($page,$limit,$sidx,$sord,$wh=""){
        ...
        if ($result = $this->mysqli->query("SELECT COUNT(*) AS count FROM ".$this->table." WHERE 1=1 ".$wh)){
            while ($row = $result->fetch_object()){
                ...
                $query="SELECT a.categorySection_id,e.place_title as place_title,d.category_title as category_title,c.section_title as section_title,a.categorysection_order,a.active,a.meta_id
                    FROM 
                    ".$this->table." a
                    LEFT JOIN
                ...
                if ($result1) {
                    ...
                    while ($row1 = $result1->fetch_object()) {
                        ...
                        $responce->rows[$i]['cell'] =array($row1->categorySection_id,$row1->place_title,$row1->category_title,$row1->section_title,$row1->categorysection_order,$row1->meta_id,$row1->active);
                        $i++;
                    }
                    return $responce;
                }
    ...
    public function addDetails($place_id,$category_id,$section_id,$categorysection_order,$meta_id,$active){
        $place_id=$this->mysqli->real_escape_string($place_id);
    ...
    public function editDetails($place_id,$category_id,$section_id,$categorysection_order,$meta_id,$active,$id){
        ...
        $result = $this->mysqli->query("UPDATE ".$this->table." SET category_id='$category_id',section_id='$section_id',categorysection_order='$categorysection_order',meta_id='$meta_id',active='$active' WHERE ".$this->id."='$id'");
        if ($result){
