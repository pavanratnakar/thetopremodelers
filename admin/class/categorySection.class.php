...
class CategorySection extends General{
    public function getDetails($page,$limit,$sidx,$sord,$wh=""){
        ...
        if ($result = $this->mysqli->query("SELECT COUNT(*) AS count FROM ".$this->table." WHERE 1=1 ".$wh)){
            while ($row = $result->fetch_object()){
                ...
                $query="SELECT a.categorySection_id,e.place_title as place_title,d.category_title as category_title,c.section_title as section_title,a.categorysection_order,a.active,a.meta_id,a.background_id
                    FROM
                    ".$this->table." a
                    LEFT JOIN
                ...
                if ($result1) {
                    ...
                    while ($row1 = $result1->fetch_object()) {
                        ...
                        $responce->rows[$i]['cell'] =array($row1->categorySection_id,$row1->place_title,$row1->category_title,$row1->section_title,$row1->categorysection_order,$row1->meta_id,$row1->background_id,$row1->active);
                        $i++;
                    }
                    return $responce;
                }
    ...
    public function addDetails($place_id,$category_id,$section_id,$categorysection_order,$meta_id,$background_id,$active){
        $place_id=$this->mysqli->real_escape_string($place_id);
        ...
        $meta_id=$this->mysqli->real_escape_string($meta_id);
            ...
            foreach($sectionIds as $sectionIdKey=>$sectionKeyValue) {
                ...
                if ($result = $this->mysqli->query($query)) {
                    while ($row = $result->fetch_object()) {
                        $count = $row->count;
                        if ($count == 0) {
                            $query1 = "INSERT INTO ".$this->table."(placeCategory_id,section_id,categorysection_order,meta_id,background_id,active) VALUES('$placeCategoryIdValue','$sectionKeyValue','$i','$meta_id','$background_id','$active')";
    ...
    public function editDetails($place_id,$category_id,$section_id,$categorysection_order,$meta_id,$background_id,$active,$id){
        $place_id=$this->mysqli->real_escape_string($place_id);
        $category_id=$this->mysqli->real_escape_string($category_id);
        $section_id=$this->mysqli->real_escape_string($section_id);
        $categorysection_order=$this->mysqli->real_escape_string($categorysection_order);
        ...
        $result = $this->mysqli->query("UPDATE ".$this->table." SET section_id='$section_id',categorysection_order='$categorysection_order',meta_id='$meta_id',background_id='$background_id',active='$active' WHERE ".$this->id."='$id'");
        if ($result){
