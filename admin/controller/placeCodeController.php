...
class placeCodeController{
    ...
    public function __construct($ref=null){
        ...
        include_once(Config::$admin_path.'/class/placeCode.class.php');
        $this->utils=new Utils();
        $this->placeCode=new PlaceCode('placeCode');
    ...
    public function details(){
        if (isset($_REQUEST['_search']) && $this->searchOn!='false') {
            ...
            if ($fld=='code' || $fld=='place_title' || $fld=='type') {
        ...
        $response=$this->placeCode->getDetails($this->page,$this->limit,$this->sidx,$this->sord,$wh);
    ...
    public function operation(){
        ...
        /* ADD */
        if ($oper=='add') {
            $response=$this->placeCode->addDetails(
                $this->utils->checkValues($_POST['code']),
        ...
        /* EDIT */
            ...
            $response=$this->placeCode->editDetails(
                $this->utils->checkValues($_POST['code']),
                $this->utils->checkValues($_POST['place_title']),
                $this->utils->checkValues($_POST['type']),
        ...
        /* DELETE */
            ...
            $response=$this->placeCode->deleteDetails($this->utils->checkValues($_POST['id']));
            if ($response) {
                $status=TRUE;
    ...
    public function select(){
        $response= $this->placeCode->getSelect($this->utils->checkValues($_GET['place_id'],$this->utils->checkValues($_GET['returnType'])));
...
if(isset($_REQUEST['ref'])){
    $placeCodeController=new PlaceCodeController($_REQUEST['ref']);
}
?>