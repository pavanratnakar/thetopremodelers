<?php
class sectionQuestionController
{
    private $utils;
    private $page;
    private $limit;
    ...
    private $sord;
    private $searchOn;
    private $sectionQuestion;
    public function __construct($ref=null){
        include_once($_SERVER['DOCUMENT_ROOT'].'/config.class.php');
        include_once(Config::$site_path.'global/Class/utils.class.php');
        include_once(Config::$admin_path.'class/sectionQuestion.class.php');
        $this->utils=new Utils();
        $this->sectionQuestion=new SectionQuestion('sectionQuestion');
    ...
    public function details(){
        if(isset($_REQUEST['_search']) && $this->searchOn!='false') {
            ...
            if( $fld=='section_id' || $fld=='section_name' || $fld=='section_value') {
        ...
        $response=$this->sectionQuestion->getDetails($this->page,$this->limit,$this->sidx,$this->sord,$wh);
    ...
    public function operation(){
        ...
        /* ADD */
        if($oper=='add'){
            $response=$this->sectionQuestion->addDetails(
                $this->utils->checkValues($_POST['question_text']),
                $this->utils->checkValues($_POST['section_name']),
                $this->utils->checkValues($_POST['question_order'])
            );
            if($response){
                $status=TRUE;
        ...
        /* EDIT */
            ...
            $response=$this->sectionQuestion->editDetails(
                $this->utils->checkValues($_POST['question_text']),
                $this->utils->checkValues($_POST['section_name']),
                $this->utils->checkValues($_POST['question_order']),
        ...
        /* DELETE */
            ...
            $response=$this->sectionQuestion->deleteDetails($this->utils->checkValues($_POST['id']));
            if($response){
                $status=TRUE;
    ...
    public function select(){
        $response= $this->sectionQuestion->getSelect();
...
if(isset($_REQUEST['ref'])){
    $sectionQuestionController=new SectionQuestionController($_REQUEST['ref']);
}
?>