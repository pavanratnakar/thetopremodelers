...
class placeCategoryController{
    ...
    public function __construct($ref=null){
        ...
        include_once(Config::$admin_path.'/class/placeCategory.class.php');
        $this->utils=new Utils();
        $this->placeCategory=new PlaceCategory('placeCategory');
    ...
    public function details(){
        if (isset($_REQUEST['_search']) && $this->searchOn!='false') {
            ...
            if ($fld=='category_title' || $fld=='place_title' || $fld=='active') {
        ...
        $response=$this->placeCategory->getDetails($this->page,$this->limit,$this->sidx,$this->sord,$wh);
    ...
    public function operation(){
        ...
        /* ADD */
        if ($oper=='add') {
            $response=$this->placeCategory->addDetails(
        ...
        /* EDIT */
            ...
            $response=$this->placeCategory->editDetails(
                $this->utils->checkValues($_POST['place_title']),
                $this->utils->checkValues($_POST['category_title']),
                $this->utils->checkValues($_POST['active']),
        ...
        /* DELETE */
            ...
            $response=$this->placeCategory->deleteDetails($this->utils->checkValues($_POST['id']));
            if ($response) {
                $status=TRUE;
    ...
    public function select(){
        $response= $this->placeCategory->getSelect($this->utils->checkValues($_GET['place_id'],$this->utils->checkValues($_GET['returnType'])));
...
if(isset($_REQUEST['ref'])){
    $placeCategoryController=new PlaceCategoryController($_REQUEST['ref']);
}
?>