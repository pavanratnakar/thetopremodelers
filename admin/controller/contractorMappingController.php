<?php
class contractorMappingController{
    private $utils;
    private $page;
    private $limit;
    private $sidx;
    private $sord;
    private $searchOn;
    private $contractorMapping;
    public function __construct($ref=null){
        include_once($_SERVER['DOCUMENT_ROOT'].'/config.class.php');
        include_once(Config::$site_path.'global/Class/utils.class.php');
        include_once(Config::$admin_path.'/class/contractorMapping.class.php');
        $this->utils=new Utils();
        $this->contractorMapping=new ContractorMapping('contractorMapping');
    ...
    public function details(){
        if (isset($_REQUEST['_search']) && $this->searchOn!='false') {
            ...
            if ($fld=='contractor_title' || $fld=='place_title' || $fld=='section_title' || $fld=='category_title' || $fld=='active') {
        ...
        $response=$this->contractorMapping->getDetails($this->page,$this->limit,$this->sidx,$this->sord,$wh);
    ...
    public function operation(){
        ...
        /* ADD */
        if($oper=='add'){
            $response=$this->contractorMapping->addDetails(
        ...
        /* EDIT */
            ...
            $response=$this->contractorMapping->editDetails(
                $this->utils->checkValues($_POST['place_title']),
                $this->utils->checkValues($_POST['category_title']), 
                $this->utils->checkValues($_POST['section_title']),
                $this->utils->checkValues($_POST['contractor_title']),
        ...
        /* DELETE */
            ...
            $response=$this->contractorMapping->deleteDetails($this->utils->checkValues($_POST['id']));
            if($response){
                $status=TRUE;
    ...
    public function select(){
        $response= $this->categorySection->getSelect();
...
if(isset($_REQUEST['ref'])){
    $contractorMappingController=new ContractorMappingController($_REQUEST['ref']);
}
?>