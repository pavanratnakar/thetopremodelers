<?php
class contractorRatingController{
    private $utils;
    private $page;
    private $limit;
    private $sidx;
    ...
    public function __construct($ref=null){
        ...
        include_once(Config::$admin_path.'/class/contractorRating.class.php');
        $this->utils=new Utils();
        $this->contractorRating=new ContractorRating('contractorRating');
    ...
    public function details(){
        if (isset($_REQUEST['_search']) && $this->searchOn!='false') {
            ...
            if ($fld=='contractor_title' || $fld=='score' || $fld=='review' || $fld=='timestamp' || $fld=='person' || $fld=='place_title' || $fld=='project') {
        ...
        $response=$this->contractorRating->getDetails($this->page,$this->limit,$this->sidx,$this->sord,$wh);
    ...
    public function operation(){
        ...
        /* ADD */
        if($oper=='add'){
            $response=$this->contractorRating->addDetails(
                $this->utils->checkValues($_POST['contractor_title']), 
                $this->utils->checkValues($_POST['score']),
                $this->utils->checkValues($_POST['review']),
                $this->utils->checkValues($_POST['timestamp']),
                $this->utils->checkValues($_POST['person']),
                $this->utils->checkValues($_POST['place_title']),
                $this->utils->checkValues($_POST['project'])
            );
            if($response){
                $status=TRUE;
        ...
        /* EDIT */
            ...
            $response=$this->contractorRating->editDetails(
                $this->utils->checkValues($_POST['contractor_title']), 
                $this->utils->checkValues($_POST['score']),
                $this->utils->checkValues($_POST['review']),
                $this->utils->checkValues($_POST['timestamp']),
        ...
        /* DELETE */
            ...
            $response=$this->contractorRating->deleteDetails($this->utils->checkValues($_POST['id']));
            if($response){
                $status=TRUE;
...
if(isset($_REQUEST['ref'])){
    $contractorRatingController=new ContractorRatingController($_REQUEST['ref']);
}
?>