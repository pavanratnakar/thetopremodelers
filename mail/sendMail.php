...
if($_REQUEST['categoryName']){
    ...
    $message .= '<hr/><b>For</b> : '.$placeDetails['place_title'].'<hr/>';
} else {
    $categoryName = null;
}
...
if($categoryName){
    $message .= '
            <br/><hr/><b>User Selection</b><hr/>
            <b>Category Selected</b> : '.$category->getCategoryValueByName($categoryName).'
            <br/><b>Section Selected</b> : '.$section->getSectionTitleByName($utils->checkValues($_REQUEST['sectionName'])).'<br/><br/>
            <hr/><b>Questions Answered</b><hr/>
