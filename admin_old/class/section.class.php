...
class Section extends General
{
            ...
            while ($row = $result->fetch_object()){
                ...
                if ($result1) {
                    ...
                    while ($row1 = $result1->fetch_object()) {
                        ...
                        $responce->rows[$i]['cell'] =array($row1->section_id,$row1->section_name,$row1->section_title,$row1->section_value);
                        $i++;
                    }
                    return $responce;
                }
    ...
    public function addDetails($section_name,$section_title,$section_value)
    {
        $section_name=$this->mysqli->real_escape_string($section_name);
        $section_title=$this->mysqli->real_escape_string($section_title);
        $section_value=$this->mysqli->real_escape_string($section_value);
        $result = $this->mysqli->query("INSERT INTO ".$this->table."(section_name,section_title,section_value) VALUES('$section_name','$section_title','$section_value')");
        if ($result) 
        {
    ...
    public function editDetails($section_name,$section_title,$section_value,$id)
    {
        $section_name=$this->mysqli->real_escape_string($section_name);
        $section_title=$this->mysqli->real_escape_string($section_title);
        $section_value=$this->mysqli->real_escape_string($section_value);
        $id=$this->mysqli->real_escape_string($id);
        $result = $this->mysqli->query("UPDATE ".$this->table." SET section_name='$section_name',section_title='$section_title', section_value='$section_value' WHERE ".$this->id."='".$id."'");
        if ($result){
            if($this->mysqli->affected_rows>0){
                return TRUE;
            }
